...
export default function TenantDayScreenshotsPage() {
  ...
  const [preview, setPreview] = useState<{ id: number; createdAt: string; sessionId: string } | null>(
    null
  );
  ...
  const loadData = async (apiKey: string) => {
    ...
    try {
      ...
      const res = await client.getScreenshotsForDay(tenantId, day, page, PAGE_SIZE);
      setItems(res.items);
      setTotal(res.total);

      const entries = await Promise.all(
        res.items.map(async (item) => {
          const url = client.getScreenshotFileUrl(tenantId, item.session_id, item.id);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-8">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{day}</h1>
        <p className="text-sm text-gray-500 mt-1">共 {total} 則問答（跨所有使用者，依時間排列）</p>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border">
        {loading ? (
          ...
          <p className="text-gray-400 text-sm py-8 text-center">當天尚無截圖記錄</p>
        ) : (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {items.map((item) => (
                <button
                  key={item.id}
                  onClick={() =>
                    setPreview({ id: item.id, createdAt: item.created_at, sessionId: item.session_id })
                  }
                ...
                >
                  ...
                  <p className="text-xs text-gray-400 truncate font-mono">{item.session_id}</p>
                </button>
              ))}
            </div>
        ...
        >
          <div className="max-w-3xl w-full">
            ...
            <p className="text-center text-white text-sm mt-3">
              {new Date(preview.createdAt).toLocaleString('zh-TW')} · {preview.sessionId}
            </p>
          </div>
        </div>
      )}
